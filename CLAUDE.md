...
- Active references are session-scoped: copied into `%APPDATA%\AIRenderer\active-references\` (deleting the source file must not break an in-flight request), kept across generations within one window, and cleared when the window closes (copies deleted, settings.json list emptied). Leftovers from a crashed session are restored at the next launch. The cap (`RenderSettings.MaxActiveReferences`, 3) is enforced at add, restore, send, and as a service-layer backstop — keep all four in sync when touching reference code. Thumbnails must bind the cached `Thumbnail` property (320px decode), never `FilePath` — a path binding makes WPF decode the full image per thumbnail.
