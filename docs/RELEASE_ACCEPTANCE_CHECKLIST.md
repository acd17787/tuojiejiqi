...
| 第二层 | 产品操作验收（真实 Rhino 里实际操作插件：走查工作主循环 + 常用动作 + 场景断言） | **全绿才可发包给客户** | 零（BaseUrl 指向本地 mock） |
...
> 本节是"当用户用它干活"。Rhino 打开任意模型（默认立方体即可），把插件当真实
> 工具从头操作到尾；每一步都写了「做什么 / 应该看到什么」。任何一步不成立，
> 第二层即不通过。驱动脚本建成后，本节就是脚本的主流程脚本。

前置：启动本地 mock（零费用跑法）或直连真 API（真实跑法，见第三层预算）。
两种跑法**步骤完全相同**，差别只在第 4 步：

| 跑法 | 第 4 步得到什么 | 验证到什么程度 |
| --- | --- | --- |
| A 零费用（mock） | mock 的 8×8 占位图 | 全部流程机制、请求形状、界面状态 |
| B 真实（真 API，第三层预算内） | 真实渲染图 | 上述全部 + 出图质量与场景对应关系 |

**工作主循环：**

1. **取原图**：Rhino 里摆好视角 → 插件点「导入模型」。
   看到：原始图像预览出现视口内容，状态条显示「原图 宽×高」。
2. **写意图**：提示词输入一句真实任务，例如
   `把这个空间渲染成暖色木质风格的室内效果图，保持视角与构图不变。`
   看到：右下角字数计数随之变化。
3. **生成**：点「生成」。
   看到：生成按钮进入禁用、进度条启动、状态条「正在生成… N%」
   （标准模式另显示「标准模式 · 尺寸 · 比例」摘要，带参考图时还有「· 参考 N 张」）。
4. **看结果**：完成。
   看到：结果预览出现、toast「生成完成」、进度文字变「完成，总耗时 Xs」、
   历史抽屉计数 +1。跑法 B 时额外检查：出图与提示词意图对应、视角未被改变。
5. **处理结果**：依次点「再次编辑」（结果回填为原图）、「下载」（存 PNG 成功）、
   「复制图片」（粘贴到画图验证）、点大图开灯箱再 Esc 关闭。
6. **参考图参与**：加一张参考图 → 状态条出现「· 参考 1 张」→ 再生成 →
   结果请求带上了参考图（跑法 A 看 mock 记录，跑法 B 看风格被参考图带偏）。
7. **蒙版修改**：标准模式点「涂抹修改」→ 涂一小块 → 完成 → 生成。
   看到：请求固定走 sunburst 模型，结果只改了涂抹区域。
8. **收尾**：设置弹层「清除原图与生成结果」→ 原图 / 结果 / 蒙版 / 参考图清空，
   历史记录保留。
...
| 动作 | 预期 |
| --- | --- |
| 快速出图 / 标准模式切换 | 选中态高亮正确；比例与尺寸档随模式启用/禁用；禁用档位有置灰观感 |
| 比例选「原图」 | 悬停提示显示吸附到的档位（如「按原图 3:2」） |
| 尺寸档 1K/2K/4K | 选中态与「输出 宽×高 · MP」摘要实时更新 |
| 画笔 / 擦除切换 | 选中态高亮；擦除能删除已有笔画 |
| 笔刷大小滑杆 | 光标圆圈实时变大小，与实际笔画粗细一致 |
| 清除蒙版 | 笔画清空，蒙版应用态复位 |
| 历史抽屉开关 | 展开收起平滑，计数与目录文件数一致 |
| 灯箱 | 点缩略图/预览打开；Esc 或点空白关闭；下载按钮仅在可下载项出现 |
| 窗口缩放 | >1120 宽窗两栏；拖到 ≤1120（含 1120 整）收窄为单列，无遮挡 |
| 窗口拖小再拉大 | 布局完全恢复，无残留错位 |
| 设置弹层开关 | 点空白处收起；Esc 不误关其它层 |
...
> 在驱动脚本建成之前，2.5 的场景由人工按步骤执行、按断言逐条勾选；
> 建成之后同一张表就是自动化脚本的场景清单。**不允许因为脚本未建而跳过本层。**

### 2.5 场景表（细化断言；2.2 走查覆盖不到的边界与异常在这里）
...
- [ ] **真实 API 抽测（预算：3 张）**：恢复 `https://api.apiyi.com/v1` + 客户 Key，
  按 2.2 走查的**跑法 B** 把主循环走一遍——第 4 / 6 / 7 步各对应一次真实生成
  （标准出图、带参考图、蒙版修改），确认客户 Key 对三个默认模型
